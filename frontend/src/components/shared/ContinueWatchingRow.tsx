    ...
    <section className="mb-8">
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
        {items.slice(0, 4).map((item) => (
          <Link key={item.movieId} href={`/watch/${item.movieId}`} className="group block rounded-[28px] border border-white/10 bg-gradient-to-br from-white/5 to-transparent p-3 transition hover:-translate-y-1 hover:border-cv-accent/40 hover:shadow-[0_20px_50px_rgba(59,130,246,0.18)]">
            <div className="relative aspect-[2/3] overflow-hidden rounded-[22px] shadow-[0_12px_30px_rgba(0,0,0,0.25)]">
              <Image src={item.posterUrl} alt={item.movieTitle} fill sizes="(max-width: 768px) 50vw, 25vw" className="object-cover transition duration-500 group-hover:scale-105" />
